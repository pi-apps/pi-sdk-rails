...
RSpec.describe Pinetwork::Rails::PiPaymentController, type: :controller do
  ...
  describe 'POST #approve' do
    # The server now checks that the access token is valid using Pinetwork::Rails::UserDTO.get.
    ...
    let(:user_dto_double) { instance_double('Pinetwork::Rails::UserDTO', uid: 'u', credentials: {}, username: nil) }
    ...
    it 'returns 401 unauthorized if token is invalid' do
      allow(::Pinetwork::Rails::UserDTO).to receive(:get).with(access_token).and_return(nil)
    ...
    it 'approves a payment and sends the API key in the Authorization header' do
      allow(::Pinetwork::Rails::UserDTO).to receive(:get).with(access_token).and_return(user_dto_double)
      request.headers['Content-Type'] = 'application/json'
      request.headers['Accept'] = 'application/json'
      post :approve, params: {
        accessToken: access_token,
