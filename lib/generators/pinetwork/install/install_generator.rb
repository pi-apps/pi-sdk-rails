...
class Pinetwork::InstallGenerator < Rails::Generators::Base
  ...
  def show_js_todo
    ...
    say "\nYou now have a pinetwork_controller.js, for customization.\n", :yellow
  end
  ...
  def inject_controller_import
    ...
    import_statement = "import 'pinetwork/pinetwork_controller'"
    ...
    unless contents.include?(import_statement)
      ...
      say "Inserted 'import 'pinetwork/pinetwork_controller'' before 'import 'controllers'' in #{js_file}", :green
    else
      say "'import 'pinetwork/pinetwork_controller'' is already present in #{js_file}", :yellow
    end
  end

  def add_importmap_pin
    importmap_file = "config/importmap.rb"
    pin_line = "pin_all_from '../../app/javascript/pinetwork/rails/controllers', under: 'pinetwork/controllers'"
